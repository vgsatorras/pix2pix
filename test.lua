...
opt = {
   DATA_ROOT = '/imatge/vgarcia/analysis_comparison/art_bw_images/original',
   find_list = '',
    ...
    --checkpoints_dir = '/imatge/vgarcia
    -- -- /analysis_comparison/models/ours_all', -- loads models from here
    checkpoint_model = '/imatge/vgarcia/analysis_comparison/models/ours_all_399300/latest_net_G.t7', -- load this model
    results_dir='/imatge/vgarcia/analysis_comparison/art_bw_images/results_399300/',          -- saves results here
...
local netG = util.load(opt.checkpoint_model, opt)
...
for n=1,math.floor(opt.how_many/opt.batchSize) do
    ...
    local data_curr, filepaths_curr, data_labels = data:getBatch()
    ...
    if opt.preprocess == 'colorization' then
        local output_both = netG:forward(input)
        local output_AB = output_both[1]
        local output_AB = output_AB:float()
        input_L = input:float()
    ...
    local image_dir = opt.results_dir
